...
public class WaitingListValidator {
    ...
    public void validateWaitingListEligibility(DormitoryApplication application) {
        if (application.getStatus() != ApplicationStatus.APPROVED) {
            throw new AppException(ErrorCode.VALIDATION_FAILED, "Admission error. Only applications with 'APPROVED' status are eligible to enter the waiting list queue.");
        }
    }
    ...
    public void validatePromotionCandidate(DormitoryApplication application) {
        if (application.getStatus() != ApplicationStatus.WAITING_LIST) {
            throw new AppException(ErrorCode.VALIDATION_FAILED, "Promotion error. Candidate application is no longer in active 'WAITING_LIST' status.");
        }

        if (Boolean.TRUE.equals(application.getWaitingListUsed())) {
            throw new AppException(ErrorCode.VALIDATION_FAILED, "Security restriction. This application has already utilized its single allowed waiting list promotion lifecycle loop.");
        }
    }
    ...
    public void validateCanExpire(StudentHousingAssignment assignment) {
        ...
        if (assignment.getStatus() != AssignmentStatus.RESERVED) {
            throw new AppException(ErrorCode.VALIDATION_FAILED, "Lifecycle error. Cannot expire housing assignment. Only 'RESERVED' accounts are eligible for resource release.");
        }
        ...
        if (application == null) {
            throw new AppException(ErrorCode.INTERNAL_SERVER_ERROR, "Data integrity violation. The housing assignment is missing its core reference to the original Dormitory Application.");
        }
        ...
        if (application.getPaymentDeadline() == null) {
            throw new AppException(ErrorCode.INTERNAL_SERVER_ERROR, "Operational failure. The active reservation is missing its mandatory payment deadline timestamp.");
        }
        ...
        if (LocalDateTime.now().isBefore(application.getPaymentDeadline())) {
            throw new AppException(ErrorCode.VALIDATION_FAILED, "Operational restriction. The designated payment deadline for this reservation has not been breached yet.");
        }
    }
}