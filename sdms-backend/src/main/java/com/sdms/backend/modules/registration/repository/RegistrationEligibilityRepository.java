...
public interface RegistrationEligibilityRepository
    ...
    @Query("SELECT e.cccd FROM RegistrationEligibility e WHERE e.registrationPeriod.periodId = :periodId")
    Set<String> findCccdByRegistrationPeriod_PeriodId(@Param("periodId") UUID periodId);

    // 3. Giữ lại phục vụ cho logic kiểm tra điều kiện (CheckEligibility) lúc sinh viên đăng ký công khai
    Optional<RegistrationEligibility> findByRegistrationPeriod_PeriodIdAndCccd(UUID periodId, String cccd);

    // 4. Kiểm tra nhanh sự tồn tại (Giữ lại nếu module khác cần dùng)
    boolean existsByRegistrationPeriod_PeriodIdAndCccd(UUID periodId, String cccd);
