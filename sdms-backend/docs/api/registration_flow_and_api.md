...
1. **~~Dư thừa Nút "Xác nhận thu tiền mặt" ở trang Duyệt đơn~~ (ĐÃ FIX ✅):**
   - **Trạng thái**: Đã xóa hoàn toàn logic thu tiền mặt khỏi `ApplicationReviewDetail.tsx` (Frontend) và `ApplicationReviewController.java` (Backend). Chức năng này đã được bàn giao hoàn toàn cho Module Payment.
