...
```mermaid
graph TD
    A[Nhận yêu cầu từ Cổng qua HTTP POST] --> B{Kiểm tra Idempotency};
...
*   **Hành động:** `IotVerificationController` nhận yêu cầu HTTP POST từ thiết bị.
...
*   **Logic:**
    ...
    3.  `IotVerificationController` tổng hợp kết quả và trả về HTTP Response JSON (ví dụ: `{"status": "GRANTED"}`) trực tiếp cho thiết bị ESP32.
*   **Đối chiếu code:**
...
## 4. Quyền Hạn và Vai Trò (Roles & Permissions)

Để đảm bảo nguyên tắc an toàn thông tin mà không làm phức tạp hóa cơ sở dữ liệu với một hệ thống RBAC động quá mức cần thiết cho đồ án, các quyền chi tiết (Granular Capabilities) của module Smart Access được cấp phát tĩnh (Statically Mapped) dựa trên `Role` của `UserAccount`.

### 4.1. Vai Trò Quản Trị Viên (ADMIN)
*   **Quyền hạn:** Có toàn quyền kiểm soát hệ thống Smart Access.
*   **Capabilities được cấp:**
    *   `MANAGE_CURFEW_POLICY`: Quản lý chính sách giờ giới nghiêm.
    *   `MANAGE_TIME_WINDOW_POLICY`: Quản lý chính sách khung giờ ra vào.
    *   `VIEW_ACCESS_HISTORY`: Xem toàn bộ lịch sử quét thẻ/khuôn mặt.
    *   `REMOTE_UNLOCK`: Bấm nút mở cổng từ xa.
    *   `EMERGENCY_OVERRIDE`: Kích hoạt chế độ Khẩn cấp (Lockdown toàn bộ hoặc Mở bung toàn bộ cổng).

### 4.2. Vai Trò Ban Quản Lý KTX / Lễ Tân (STAFF)
*   **Quyền hạn:** Có quyền theo dõi và hỗ trợ sinh viên trong các hoạt động ra vào hàng ngày, nhưng **KHÔNG** được quyền thay đổi các chính sách hệ thống hay kích hoạt lệnh khẩn cấp.
*   **Capabilities được cấp:**
    *   `VIEW_ACCESS_HISTORY`: Xem lịch sử quét thẻ/khuôn mặt để kiểm tra tình trạng ra vào của sinh viên.
    *   `REMOTE_UNLOCK`: Bấm nút mở cổng từ xa (trong trường hợp sinh viên mất thẻ/chưa duyệt khuôn mặt).
*   **Hạn chế:** Nếu một STAFF cố tình gọi API `/api/v1/access/emergency` hoặc `/api/v1/access/curfew-policies`, hệ thống Spring Security sẽ trả về lỗi `403 Forbidden` do thiếu `EMERGENCY_OVERRIDE` hoặc `MANAGE_CURFEW_POLICY`.
