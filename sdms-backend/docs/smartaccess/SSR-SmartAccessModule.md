...
- **[FR-ACCESS-001] Tiếp nhận Yêu cầu từ IoT:**
    - **Mô tả:** Hệ thống **Phải** có khả năng tiếp nhận các yêu cầu xác thực (chứa dữ liệu khuôn mặt hoặc mã RFID) từ các thiết bị IoT thông qua giao thức MQTT.
    - **Tiền điều kiện:** Thiết bị IoT kết nối và gửi tin nhắn đến đúng topic MQTT.
...
- **[FR-ACCESS-005] Ra quyết định và Gửi lệnh:**
    - **Mô tả:** Dựa trên kết quả của các bước đánh giá, hệ thống **Phải** đưa ra quyết định cuối cùng (`GRANTED` hoặc `DENIED`) và gửi lệnh điều khiển (ví dụ: `OPEN_DOOR`) về lại cho thiết bị IoT qua MQTT.
    - **Tiền điều kiện:** Toàn bộ chuỗi đánh giá đã hoàn tất.
    - **Hậu điều kiện:** Lệnh được gửi đi.
