...
const PRIORITY_DOC_MAP = {
  ...
  PRIORITY_03: { type: 'PRIORITY_03_PROOF', label: 'Minh chứng Dân tộc thiểu số', desc: 'Giấy khẳng định / Hộ khẩu' },
...
const DocumentUploadBox = ({ doc, isUploaded, uploadedName, loading, onUpload }) => {
  ...
  return (
    <Box 
      sx={{ 
        ...
        bgcolor: (theme) => isUploaded ? alpha(theme.palette.success.main, 0.05) : 'background.default',
      ...
      component="label"
      title={isUploaded ? uploadedName : undefined} 
    >
      <input 
        type="file" 
        hidden 
      ...
      {isUploaded ? (
        ...
        <>
          ...
          <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>{doc.label}</Typography>
...
export default function DocumentUploadSection({ error, uploadedDocs, handleUpload, loading, formData }) {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, maxWidth: 600, mx: 'auto', mt: 4 }}>
      ...
      {extraDocs.length > 0 && (
        <>
          <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mt: 2, color: 'secondary.main' }}>
