...
export default function RegistrationPage() {
  const { 
    activeStep, loading, error, period, targetGroup, formData, 
    setFormData, uploadedDocs, handleNext, handleBack, handleUpload 
  } = useRegistration();
  ...
  const renderStepSection = useMemo(() => {
    switch (activeStep) {
      ...
      case 1:
        return <InfoSection period={period} targetGroup={targetGroup} formData={formData} setFormData={setFormData} error={error} />;
      case 2:
        return (
  ...
  }, [activeStep, error, formData, period, targetGroup, uploadedDocs, handleUpload, loading, setFormData]);

  // Caching text header, không tính toán chuỗi rỗng khi điền Form
  const periodSubTitle = useMemo(() => {
    if (!period) return null;
    const typeText = REGISTRATION_TYPE_TEXT[period.registrationType] || period.registrationType;
    ...
    <Container maxWidth="md" sx={{ py: 8 }}>
      <Fade in timeout={800}>
        ...
        >
          ...
          <Stepper activeStep={activeStep} alternativeLabel sx={{ mb: 6, mt: periodSubTitle ? 0 : 3 }}>
