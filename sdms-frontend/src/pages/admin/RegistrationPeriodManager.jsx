import { Container, Typography, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, TextField, MenuItem, Snackbar, Alert, Box, CircularProgress, Chip, Tooltip } from '@mui/material';
            ...
            {periods.length === 0 ? (
              ...
              periods.map((row) => (
                <TableRow key={row.periodId} hover>
                  <TableCell sx={{ fontWeight: 500, maxWidth: 220, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    <Tooltip title={row.periodName} placement="top-start">
                      <span>{row.periodName}</span>
                    </Tooltip>
                  </TableCell>
                  <TableCell>{REGISTRATION_TYPES[row.registrationType] || row.registrationType}</TableCell>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 0.5 }}>
                      {row.registrationType !== 'OPEN_REGISTRATION' && (
                        <Tooltip title="Danh sách đủ điều kiện">
                          <IconButton color="info" size="small" onClick={handleOpenEligibility(row)}>
        ...
        <DialogContent dividers>
          <Box sx={{ pt: 1 }}>
            <Grid container spacing={2.5}>
              <Grid size={{ xs: 12 }}>
                <TextField fullWidth label="Tên đợt" value={formData.periodName} onChange={handleFormChange('periodName')} required />
              </Grid>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField fullWidth type="datetime-local" label="Ngày bắt đầu" value={formData.startDate} onChange={handleFormChange('startDate')} slotProps={{ inputLabel: { shrink: true } }} required />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField fullWidth type="datetime-local" label="Ngày kết thúc" value={formData.endDate} onChange={handleFormChange('endDate')} slotProps={{ inputLabel: { shrink: true } }} required />
              </Grid>
            </Grid>
          </Box>
        </DialogContent>
