import { Box, Typography, Paper, Button, Chip, Card, CardMedia, CardContent, CardActions, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Snackbar, Alert } from '@mui/material';
...
export default function FaceApprovalQueue() {
  ...
  const onApproveClick = (profileId) => () => {
    handleApprove(profileId);
  };

  const onRejectClick = (profileId) => () => {
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" sx={{ color: 'text.secondary', mb: 4 }}>
        Ảnh chân dung được duyệt sẽ dùng làm mẫu AI nạp vào hệ thống Edge điều khiển mở cổng tự động.
      </Typography>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={profile.profileId}>
              <Card variant="outlined" sx={{ borderRadius: 3, boxShadow: 'none' }}>
                ...
                <CardContent sx={{ p: 2 }}>
                  ...
                  <Chip label="Chờ xét duyệt" size="small" color="warning" sx={{ fontWeight: 600, borderRadius: 1.5 }} />
                </CardContent>
      ...
      <Dialog open={Boolean(rejectTarget)} onClose={() => setRejectTarget(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          ...
          <Button onClick={handleRejectSubmit} variant="contained" color="error" disabled={!reason.trim()}>
            Xác Nhận
          </Button>
        </DialogActions>
      </Dialog>
