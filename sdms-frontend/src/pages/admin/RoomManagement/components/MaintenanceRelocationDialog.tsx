  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: '24px', p: 1 } }}>
        ...
        {loading ? (
          ...
          <Stack spacing={3}>
            {assignments.map((assignment) => {
              ...
              return (
                <Box key={student.studentId} sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'background.default' }}>
                  <Typography variant="subtitle1" fontWeight="bold" mb={1}>
                    {student.fullName} - {student.studentCode}
                  </Typography>
                  <Typography variant="body2" color="text.secondary" mb={2}>
                    Đang ở: Giường {assignment.bedCode}
                  </Typography>
      ...
      <DialogActions sx={{ pt: 2, px: 3 }}>
        <Button onClick={onClose} color="inherit" disabled={relocating}>
          Hủy
        </Button>
        <Button 
          variant="contained" 
