import { Box, Typography, Paper, TextField, Button, Avatar, Divider, Chip, CircularProgress, Alert } from '@mui/material';
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 5 }}>
          <Paper variant="outlined" sx={{ p: 4, borderRadius: 4, height: '100%' }}>
            ...
            <TextField
              ...
              helperText={isLengthInvalid ? `Độ dài: ${currentLength} số (Yêu cầu 9 hoặc 12 số).` : ''}
              autoFocus
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Paper variant="outlined" sx={{ p: 4, borderRadius: 4, height: '100%', minHeight: 400, display: 'flex', flexDirection: 'column' }}>
            {!studentData ? (
              <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexGrow: 1, opacity: 0.5 }}>
                ...
                <Typography variant="body2">Vui lòng nhập số định danh chuẩn ở khung bên trái</Typography>
              </Box>
            ) : (
              <Box sx={{ animation: 'fadeIn 0.5s' }}>
                <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 3, color: 'primary.main', borderBottom: '2px solid', borderColor: 'primary.light', pb: 1, display: 'inline-block' }}>
                  Kết Quả Đối Chiếu
                </Typography>
                ...
                <Grid container spacing={4}>
                  ...
                  <Grid size={{ xs: 12, sm: 4 }} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <Avatar
                      ...
                      sx={{ width: 150, height: 200, mb: 2, border: '4px solid', borderColor: 'background.paper', boxShadow: 3 }}
                    >
                      Chưa có ảnh
                    </Avatar>
                    <Chip label="Ảnh gốc hồ sơ" color="info" size="small" sx={{ fontWeight: 600 }} />
                  </Grid>
                  ...
                  <Grid size={{ xs: 12, sm: 8 }}>
                    ...
                    <Box sx={{ bgcolor: (theme) => alpha(theme.palette.success.main, 0.05), p: 2, borderRadius: 3, border: '1px solid', borderColor: (theme) => alpha(theme.palette.success.main, 0.2) }}>
                      <Typography variant="subtitle2" sx={{ color: 'success.dark', display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                        <Hotel fontSize="small" /> Vị trí phòng ở được cấp:
                      </Typography>
                      <Typography variant="body1" sx={{ fontWeight: 'bold', color: 'success.main', fontSize: '1.1rem' }}>
