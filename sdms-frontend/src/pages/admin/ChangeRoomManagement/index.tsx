...
export default function ChangeRoomManagementPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      enqueueSnackbar(error.response?.data?.message || 'Lỗi khi tải danh sách yêu cầu', {
        variant: 'error',
      });
    } finally {
      setLoading(false);
  ...
  const handleProcessOpen = async (request: ChangeRoomResponseDto, isApproved: boolean) => {
    ...
    if (isApproved && request.targetRoomId) {
      ...
      try {
        ...
        enqueueSnackbar('Lỗi khi tải danh sách giường của phòng đích', { variant: 'error' });
      } finally {
  ...
  const handleProcessSubmit = async () => {
    ...
    if (processData.isApproved && !processData.newBedId) {
      enqueueSnackbar('Vui lòng nhập ID giường mới để chuyển sinh viên đến', {
        variant: 'warning',
      });
      return;
    }
        ...
        <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
          <CardContent>
            <Alert severity="info" sx={{ mb: 3 }}>
              Tính năng này cho phép di dời hàng loạt sinh viên từ một phòng đang bảo trì sang các
              giường khác. Yêu cầu phòng cũ phải có trạng thái MAINTENANCE.
            </Alert>
            ...
            {processData.isApproved && selectedRequest?.targetRoomId ? (
              <FormControl fullWidth required disabled={loadingBeds}>
                ...
                <Select
                  ...
                  onChange={(e) => setProcessData({ ...processData, newBedId: e.target.value as string })}
                >
                  {availableBeds.length === 0 && !loadingBeds && (
                    <MenuItem value="" disabled>Không có giường trống</MenuItem>
                  )}
