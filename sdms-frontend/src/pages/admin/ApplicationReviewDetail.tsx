...
import {DOCUMENT_TYPE_LABELS, PRIORITY_CATEGORY_LABELS} from '@/helpers/documentHelper';
        ...
        <Box sx={{p: {xs: 2, md: 3}}}>
                ...
                <Grid size={{xs: 12, md: 4}}>
                    <Paper variant="outlined" sx={{p: 3, borderRadius: 2}}>
                        ...
                        <List dense sx={{p: 0}}>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">Họ tên</Typography>}
                                    secondary={<Typography variant="body2"
                                                           fontWeight={500}>{app.fullName}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">MSSV</Typography>}
                                    secondary={<Typography variant="body2"
                                                           fontFamily="monospace">{app.studentCode || 'Không có'}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">CCCD</Typography>}
                                    secondary={<Typography variant="body2"
                                                           fontFamily="monospace">{app.cccd}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText primary={<Typography variant="caption" color="text.secondary">Ngày
                                    sinh</Typography>} secondary={<Typography variant="body2">{app.dob}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">Giới
                                        tính</Typography>}
                                    secondary={<Typography
                                        variant="body2">{app.gender === 'MALE' ? 'Nam' : 'Nữ'}</Typography>}
                                />
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText primary={<Typography variant="caption" color="text.secondary">Số điện
                                    thoại</Typography>}
                                              secondary={<Typography variant="body2">{app.phone}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">Email</Typography>}
                                    secondary={<Typography variant="body2">{app.email}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">Hộ khẩu thường
                                        trú</Typography>}
                                    secondary={<Typography variant="body2">{app.permanentAddress}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText primary={<Typography variant="caption" color="text.secondary">Địa chỉ liên
                                    hệ</Typography>} secondary={<Typography
                                    variant="body2">{app.contactAddress}</Typography>}/>
                            </ListItem>
                            <ListItem disableGutters>
                                <ListItemText
                                    primary={<Typography variant="caption" color="text.secondary">Diện ưu
                                        tiên</Typography>}
                                    secondary={
                                        <Typography variant="body2" fontWeight={500}
                                                    color={app.priorityCategories?.length ? 'primary.main' : 'text.primary'}>
                                            {(app.priorityCategories?.length ?? 0) > 0
                                                ? (app.priorityCategories ?? []).map(code => PRIORITY_CATEGORY_LABELS[code] || code).join(', ')
                                                : 'Không có'}
                                        </Typography>
                                    }
                                />
                    ...
                    {app.assignment && (
                        <Paper variant="outlined" sx={{
                            ...
                            <List dense sx={{p: 0}}>
                                <ListItem disableGutters>
                                    <ListItemText primary={<Typography variant="caption" color="text.secondary">Tòa
                                        nhà</Typography>} secondary={<Typography variant="body2"
                                                                                 fontWeight={500}>{app.assignment.buildingName}</Typography>}/>
                                </ListItem>
                                <ListItem disableGutters>
                                    <ListItemText
                                        primary={<Typography variant="caption" color="text.secondary">Tầng</Typography>}
                                        secondary={<Typography variant="body2"
                                                               fontWeight={500}>Tầng {app.assignment.floorName}</Typography>}/>
                                </ListItem>
                                <ListItem disableGutters>
                                    <ListItemText primary={<Typography variant="caption"
                                                                       color="text.secondary">Phòng</Typography>}
                                                  secondary={<Typography variant="body2"
                                                                         fontWeight={500}>{app.assignment.roomName}</Typography>}/>
                                </ListItem>
                                <ListItem disableGutters>
                                    <ListItemText primary={<Typography variant="caption"
                                                                       color="text.secondary">Giường</Typography>}
                                                  secondary={<Typography variant="body2"
                                                                         fontWeight={500}>{app.assignment.bedName}</Typography>}/>
                                </ListItem>
                            </List>
                        </Paper>
                    )}
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid size={{xs: 12}}>
                            <Paper variant="outlined" sx={{p: 3, borderRadius: 2}}>
                                ...
                                >
                                    <PictureAsPdf fontSize="small" sx={{mr: 1, color: 'error.main'}}/> Tài liệu sinh tự
                                    động (Đối chiếu)
                                </Typography>
                                <Divider sx={{mb: 3}}/>
                        ...
                        <Grid size={{xs: 12}}>
                            <Paper variant="outlined" sx={{p: 3, borderRadius: 2}}>
                                <Typography
                                    ...
                                    sx={{fontWeight: 600, mb: 2, display: 'flex', alignItems: 'center'}}
                                >
                                    <Assignment fontSize="small" sx={{mr: 1, color: 'primary.main'}}/> Minh chứng đính
                                    kèm (Sinh viên nộp)
                                </Typography>
                                <Divider sx={{mb: 3}}/>
                                <Grid container spacing={2}>
                                    {app.documents && app.documents.length > 0 ? (
                                                    ...
                                                    >
                                                        ...
                                                        <CardContent sx={{p: 1.5, '&:last-child': {pb: 1.5}}}>
                                                            <Typography
                                                                ...
                                                                title={DOCUMENT_TYPE_LABELS[doc.documentType] || doc.documentType}
                                                            >
                                                                {DOCUMENT_TYPE_LABELS[doc.documentType] || doc.documentType} {isInvalid && '(Sai)'}
                                                            </Typography>
                                                            <Box
                                                                sx={{
                                                                    display: 'flex',
                                        ...
                                        <Typography variant="body2"
                                                    sx={{color: 'text.secondary', p: 1, fontStyle: 'italic'}}>
            ...
            >
                ...
                <DialogActions sx={{px: 3, py: 2}}>
                    <Button onClick={() => toggleDialog('reject', false)}
                            sx={{borderRadius: 1.5, textTransform: 'none', fontWeight: 500, color: 'text.secondary'}}>Hủy
                        bỏ</Button>
                    <Button
            ...
            >
                ...
                <DialogActions sx={{px: 3, py: 2}}>
                    <Button onClick={() => toggleDialog('docVerify', false)}
                            sx={{borderRadius: 1.5, textTransform: 'none', fontWeight: 500, color: 'text.secondary'}}>Hủy
                        bỏ</Button>
                    <Button
            ...
            >
                ...
                <DialogActions sx={{px: 3, py: 2}}>
                    <Button onClick={() => toggleDialog('revision', false)}
                            sx={{borderRadius: 1.5, textTransform: 'none', fontWeight: 500, color: 'text.secondary'}}>Hủy
                        bỏ</Button>
                    <Button
