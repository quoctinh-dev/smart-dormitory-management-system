 ...
 * Đồng bộ với Design System của hệ thống
 */
export default function CustomSkeleton({ type = "card", count = 1 }) {
    
    // Helper để tạo các Skeleton đồng bộ (animation wave là chuẩn UX hiện đại)
                ...
                return (
                    <Box sx={{ pt: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                            {Array.from(new Array(count || 3)).map((_, i) => (
            ...
            case "form":
                return (
                    <Paper sx={{ p: 4, width: '100%', maxWidth: 600, mx: 'auto', mt: 4 }}>
                        ...
                        <Stack spacing={3}>
                            <SkeletonItem variant="rectangular" height={56} sx={{ borderRadius: 2 }} />
                            <SkeletonItem variant="rectangular" height={56} sx={{ borderRadius: 2 }} />
                            <SkeletonItem variant="rectangular" height={56} sx={{ borderRadius: 2 }} />
    ...
    return (
        <Box 
            sx={{ 
                ...
                '& .MuiSkeleton-root': { bgcolor: 'grey.200' } // Tinh chỉnh màu skeleton nhẹ nhàng hơn
            }}
        >
            {renderSkeleton()}
        </Box>
